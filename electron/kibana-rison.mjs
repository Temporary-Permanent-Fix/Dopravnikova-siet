// Kept as a standalone ES module (rather than inline in electron/kibana-poll.js)
// purely so it can be unit-tested directly under Node via `npm test` against
// real Kibana `_g`/`_a` shapes, without adding a browser/DOM test harness to
// this zero-dependency project.
//
...
// equivalent `bool` DSL from those so it merges cleanly into buildQueryBody()
// the same as any other filter, instead of being dropped or producing an
// invalid empty-object query.
...
// Exposed separately from parseKibanaDiscoverUrl so callers can distinguish
// "not on Discover at all" from "on Discover, but it hasn't written its
// _g/_a state yet" (both surface as parseKibanaDiscoverUrl returning null).
export function isDiscoverPath(urlString) {
  ...
  return isDiscoverUrl(url);
}

// Reads Kibana's `_g` (global: time range + auto-refresh) and `_a` (app:
// query + filters + data view) state out of the *live* URL of the embedded
// Kibana Discover view (electron/main.mjs's WebContentsView), i.e. whatever
// the operator has currently searched/filtered for themselves in Kibana.
//
// Returns `null` (never throws) when the URL isn't a Discover app URL, or
// when Discover hasn't finished writing `_g`/`_a` state yet — both are
// expected, transient states while the operator is navigating, not errors.
export function parseKibanaDiscoverUrl(urlString) {
  ...
  // `_a.dataSource.dataViewId` instead. Check both.
