...
import { parseRison, parseKibanaResultUrl } from '../src/kibana-rison.mjs';
...
function kibanaUrl({ g, a, hash = true }) {
  const base = 'https://kibana.example.com/app/discover';
  const params = [];
...
test('parseKibanaResultUrl extracts time range, refresh interval, query and filters from a hash-based URL', () => {
  ...
  const a = "(filters:!((meta:(disabled:!f,negate:!f),query:(match_phrase:(namespace_name:'logistics'))))," +
    "index:'idx-id',query:(language:kuery,query:'AgentName:DS24S26'))";
  const parsed = parseKibanaResultUrl(kibanaUrl({ g, a }));
...
test('parseKibanaResultUrl reads the data view id from _a.dataSource.dataViewId (newer Discover URL shape)', () => {
  const a = "(dataSource:(dataViewId:'840faa25-12c2-4793-bc94-594f0e55c937',type:dataView)," +
    "query:(language:kuery,query:''))";
  const parsed = parseKibanaResultUrl(kibanaUrl({ a }));
  assert.equal(parsed.dataViewId, '840faa25-12c2-4793-bc94-594f0e55c937');
});

test('parseKibanaResultUrl prefers the legacy _a.index field over dataSource.dataViewId when both are present', () => {
  const a = "(dataSource:(dataViewId:'newer-id',type:dataView),index:'legacy-id')";
  const parsed = parseKibanaResultUrl(kibanaUrl({ a }));
  assert.equal(parsed.dataViewId, 'legacy-id');
});

test('parseKibanaResultUrl reconstructs a bool query from an OR "combined" filter group', () => {
...
test('parseKibanaResultUrl reconstructs a bool query from an AND "combined" filter group', () => {
...
test('parseKibanaResultUrl drops a "combined" filter whose group has no usable sub-queries', () => {
  const a = "(filters:!((meta:(negate:!f,disabled:!f,type:combined,relation:OR,params:!()),query:())))";
  const parsed = parseKibanaResultUrl(kibanaUrl({ a }));
  assert.deepEqual(parsed.filters, []);
});

test('parseKibanaResultUrl works with a plain query-string (non-hash) URL', () => {
  const g = '(refreshInterval:(pause:!t,value:0),time:(from:now-1h,to:now))';
  const parsed = parseKibanaResultUrl(kibanaUrl({ g, hash: false }));
...
test('parseKibanaResultUrl applies partial state when only _g or only _a is present', () => {
  const onlyG = parseKibanaResultUrl(kibanaUrl({ g: '(time:(from:now-30m,to:now))' }));
  assert.deepEqual(onlyG.timeRange, { from: 'now-30m', to: 'now' });
  assert.equal(onlyG.query, null);
  assert.deepEqual(onlyG.filters, []);

  const onlyA = parseKibanaResultUrl(kibanaUrl({ a: "(query:(language:lucene,query:'foo'))" }));
...
test('parseKibanaResultUrl rejects URLs with neither _g nor _a', () => {
  assert.throws(() => parseKibanaResultUrl('https://kibana.example.com/app/discover'), /no _g or _a/);
});

test('parseKibanaResultUrl rejects non-URL input', () => {
  assert.throws(() => parseKibanaResultUrl(''));
  assert.throws(() => parseKibanaResultUrl('not a url at all'));
});

test('parseKibanaResultUrl also decodes fully percent-encoded parameter values', () => {
  ...
  const parsed = parseKibanaResultUrl(url);
